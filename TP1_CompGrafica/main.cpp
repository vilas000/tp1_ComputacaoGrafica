...
/* =========================================================
   ...
   ========================================================= */
const float ESPESSURA_MIN = 0.0003f;
const float ESPESSURA_MAX = 0.0015f;
...
glm::vec3 gradiente(float t)
{
    ...
    glm::vec3 azulEscuro(0.1f, 0.15f, 0.5f);
    glm::vec3 azul(0.15f, 0.35f, 0.7f);
    glm::vec3 ciano(0.2f, 0.55f, 0.6f);
    glm::vec3 verde(0.2f, 0.7f, 0.3f);
    glm::vec3 verdeLimao(0.5f, 0.9f, 0.2f);
    glm::vec3 amareloEsverdeado(0.8f, 0.95f, 0.2f);
    glm::vec3 laranja(1.0f, 0.5f, 0.0f);

    if (t < 0.15f)
        return glm::mix(azulEscuro, azul, t / 0.15f);
    else if (t < 0.3f)
        return glm::mix(azul, ciano, (t - 0.15f) / 0.15f);
    else if (t < 0.5f)
        return glm::mix(ciano, verde, (t - 0.3f) / 0.2f);
    else if (t < 0.7f)
        return glm::mix(verde, verdeLimao, (t - 0.5f) / 0.2f);
    else if (t < 0.85f)
        return glm::mix(verdeLimao, amareloEsverdeado, (t - 0.7f) / 0.15f);
    else
        return glm::mix(amareloEsverdeado, laranja, (t - 0.85f) / 0.15f);
}
...
/* =========================================================
   GERAR CÍRCULO PARA JUNÇÕES SUAVES
   ========================================================= */
void gerarCirculo(std::vector<float> &vboData, glm::vec3 centro, float raio, glm::vec3 cor, int segmentos = 16)
{
    for (int i = 0; i < segmentos; i++)
    {
        ...
        vboData.push_back(centro.x);
        vboData.push_back(centro.y);
        vboData.push_back(centro.z);
        vboData.push_back(cor.r);
        vboData.push_back(cor.g);
        vboData.push_back(cor.b);

        vboData.push_back(centro.x + raio * cos(ang1));
        vboData.push_back(centro.y + raio * sin(ang1));
        vboData.push_back(centro.z);
        vboData.push_back(cor.r);
        vboData.push_back(cor.g);
        vboData.push_back(cor.b);

        vboData.push_back(centro.x + raio * cos(ang2));
        vboData.push_back(centro.y + raio * sin(ang2));
        vboData.push_back(centro.z);
        vboData.push_back(cor.r);
        vboData.push_back(cor.g);
        vboData.push_back(cor.b);
...
int main(int argc, char **argv)
{
    ...
    // Primeiro: desenhar os segmentos (quads)
    ...
    // Segundo: calcular o raio e cor máximos em cada ponto de junção
    std::vector<float> raioMaxPonto(A.pontos.size(), 0.0f);
    std::vector<glm::vec3> corPonto(A.pontos.size(), glm::vec3(0.0f));

    for (const auto &s : A.segmentos)
    {
        float raio = glm::mix(ESPESSURA_MIN, ESPESSURA_MAX, s.t);
        if (raio > raioMaxPonto[s.a])
        {
            raioMaxPonto[s.a] = raio;
            corPonto[s.a] = s.cor;
        }
        if (raio > raioMaxPonto[s.b])
        {
            raioMaxPonto[s.b] = raio;
            corPonto[s.b] = s.cor;
        }
    }

    // Terceiro: gerar círculos em cada junção para suavizar transições
    for (size_t i = 0; i < A.pontos.size(); i++)
    {
        if (raioMaxPonto[i] > 0.0f)
        {
            gerarCirculo(vboData, A.pontos[i].pos, raioMaxPonto[i], corPonto[i], 12);
        }
    }
